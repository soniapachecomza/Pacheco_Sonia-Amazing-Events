  ...
  if (pastCard.length > 0) {
    ...
    for (card of pastCard) {
      ...
      <div class="flexcardImg">
        <img src=${card.image} class="flexcardimgItem" alt=${card.name
        .split(" ")
        .join("_")}>
      </div>
      <div class="card-body">
      ...
      <div class="price mt-1 mb-1">
        ...
        <button type="submit" class="buttonpink m-2 mb-5">
          <a href="./details.html?id=${
            card._id
          }" class="btn text-white  color-btn align-self-end">
            Details
          </a>
        </button>
      </div>       
...
function cargarCheck(categorys, container) {
  ...
  for (check of categorys) {
    ...
    checkDiv.innerHTML = `<input class="form-check-input" name="category" type="checkbox" id=${check
      .split(" ")
      .join("_")}
